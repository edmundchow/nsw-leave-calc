...
| `sw.js` | 55 | Service worker (cache-first with background refresh, version `v25` — bump when app files change) |
| `tax-rates.json` | 20 | Tax brackets and withholding rates (fetched on load, cached by SW) |
...
1. **Service worker cache** — `sw.js` has a `CACHE_VERSION` constant (e.g. `v24`). Strategy: **cache-first with background network refresh** — the app loads instantly from cache, then updates the cache in the background when online. Whenever `index.html`, `app.js`, `style.css`, `tax-rates.json`, or other cached assets are modified, bump this version so the browser installs the new files and purges old caches. A hard refresh (`Cmd+Shift+R` / `Ctrl+Shift+R`) forces immediate pickup.
