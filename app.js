...
function setDropdownDate(prefix, dateStr) {
    ...
    if (dayEl) dayEl.value = date.getDate();
    if (monthEl) monthEl.value = date.getMonth();
...
function calculateLeave() {
    ...
    if (mode === 'startDate') {
        ...
        const startingBalance = parseFloat(document.getElementById('startBalance').value) || 0;
        annualAccrued = startingBalance + (weeksSince * (4 / 52) * weeklyHours);
    }
...
function calculateResignation() {
    ...
    if (rate <= 0 || annualBalance <= 0) {
        document.getElementById('resignationResults').innerHTML = `<p style="color:red;">Please set rate and check balance.</p>`;
        return;
    }
    ...
    document.getElementById('resignationResults').innerHTML = `
        <div style="background:#fff3f3; border:1px solid #dc3545; padding:15px; border-radius:8px; margin-top:10px;">
            ...
            <p>⚠️ <b>Super Lost:</b> <span style="color:#dc3545;">-$${superLost.toFixed(2)}</span></p>
            <p>🎁 <b>Bonus Leave:</b> +${bonusAccrualHours.toFixed(2)} hrs ($${bonusValue.toFixed(2)})</p>
            <hr>
            <p style="font-size:1.1em; color:#28a745;"><b>"Run-Down" Advantage: +$${totalAdvantage.toLocaleString()}</b></p>
        </div>`;
}

// 7. DATA PERSISTENCE (RESTORED MANUAL BALANCE TOGGLE)
function toggleMode() {
    const mode = document.getElementById('calcMode').value;
    const section = document.getElementById('balanceSection');
    if (section) {
        section.style.display = mode === 'knownBalance' ? 'block' : 'none';
    }
    calculateLeave(); 
    saveToDB();
}
...
function loadFromDB() {
    db.transaction("userData", "readonly").objectStore("userData").get("profile").onsuccess = (e) => {
        ...
        if (d.roster) d.roster.forEach((c, i) => {
        ...
        // CRITICAL: Ensure the Manual Balance section visibility is updated after load
        toggleMode();
    };
}

// 8. LEAVE HISTORY & OPTIMIZER
...
function optimizeLeave() {
    ...
    hData.forEach(h => {
        ...
        const day = h.getDay();
        const dateStr = h.toLocaleDateString('en-AU', { day: 'numeric', month: 'short' });
        if (day === 2) tips.push({ title: `Bridge: ${dateStr}`, desc: `Take Mon off`, mult: "4 days off / 1 day leave" });
        if (day === 4) tips.push({ title: `Bridge: ${dateStr}`, desc: `Take Fri off`, mult: "4 days off / 1 day leave" });
        if (day === 3) tips.push({ title: `Mid-week: ${dateStr}`, desc: `Take Mon+Tue`, mult: "5 days off / 2 days leave" });
    });
    resultsDiv.innerHTML = tips.map(t => `<div class="opt-item"><span class="opt-tag">${t.title}</span><br>${t.desc}<br><small><b>${t.mult}</b></small></div>`).join('') || "No clusters found.";
}
